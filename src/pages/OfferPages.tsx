...
export const OfferDetailPage = ({ locale, slug }: { locale: Locale; slug: string }) => {
  ...
  return <>
    ...
    {schema && <EmbeddedPackageCheckout locale={locale} slug="schema" isSuccess={new URLSearchParams(location.search).get("success") === "1"} intakePath={`/intake?locale=${locale}&flow=checkout&product=schema`} loaded product={{ title: offer.title, price: offer.price, bullets: offer.bullets }} />}
  </>;
};
